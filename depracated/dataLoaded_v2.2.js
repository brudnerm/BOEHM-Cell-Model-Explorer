...
function dataLoaded(err, data) {
    ...
    var nest = makeNest(data, "primaryDisease", "Subtype", "subSource", "cellLineName")
    ...
    nest.forEach(function (d) {
        ...
        Subtype.forEach(function (e) {
            ...
            var subSource = e.values;

            subSource.forEach(function (f) {

                f.parent = e.key
                f.count = f.values.length;
                
                var cellLineName = f.values

                cellLineName.forEach(function (g) {
                    
                    g.parent = f.key
                    g.count = g.values.length;
                    fullCount.push(g.values.length)
                    
                    newData.push({
                        sequence: d.key + ";;" + e.key + ";;" + f.key + ";;" + g.key,
                        size: g.values.length,
                        values: g.values
